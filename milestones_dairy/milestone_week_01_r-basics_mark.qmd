...
animals
glimpse(animals)
animals %>% summary(id_animal)
number_of_rows <- nrow(animals)
age_left <- mean(animals$ age_left)
# load dplyr library
library(dplyr)
counts <- animals %>%
  count(breed, sex, location_list, sort=TRUE)
view(counts)
...
excel_excuses <- skim(animals)
excel_excuses
write_csv(excel_excuses,"excel_excuses.csv")
# Set up the summary for skim()
