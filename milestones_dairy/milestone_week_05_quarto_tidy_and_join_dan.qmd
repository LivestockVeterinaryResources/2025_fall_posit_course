...
ggplot()+
  ...
  labs(fill = '', 
       color = '', 
       shape = '',
       title = 'Denominator Options By 90 Day Intervals')+
  scale_shape_manual(values = c(21,22,24,25))
...
```{r}
events_health |> 
  filter(event == c('DA', 'KETOSIS', 'RP', 'PROLAPS')) |> 
  mutate(floor_date = floor_date(date_event, 'months')) |> 
  relocate(id_animal, event, date_event) |> 
  fxn_add_event_counts() |> 
  mutate(age_event = date_event - date_birth) |> 
  mutate(Disease = event) |> 
  
          ggplot(aes(floordate_month))+
          geom_bar()+
          facet_wrap(facet = vars(Disease))
...
Animals_with_deno <- left_join(animals, animal_lactations |> 
                               select(id_animal) |> 
                                 distinct())
  

Ani_with_deno_events <- full_join(Animals_with_deno, events_health)
