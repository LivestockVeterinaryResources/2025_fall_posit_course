...
events_formatted <- read_parquet('C:/Users/srmh0/OneDrive/Documents/GitHub/2025_fall_posit_course/data/intermediate_files/events_formatted.parquet')

dim_at_bred <- add_new_variables(events_formatted)
```
...
dim_at_bred <- events_formatted %>%
  # limit to BRED1 events only
  filter(event == "BRED1") %>%
  
  # restrict to the past 3 years from today
  filter(date_event >= today() - years(3)) %>%
  
  # create first-of-month date
  mutate(floordate_month = floor_date(date_event, unit = "month")) %>%
  
  # group by location, lactation group, and month
  group_by(location_event, lact_group, floordate_month) %>%
...
ggplot(
  data = dim_at_bred,
       mapping = aes(x = floordate_month, y = q97, 
          fill = lact_group)) +
  
  geom_ribbon(aes(ymin = q97 - sd, ymax = q97 + sd),
              alpha = 0.15, color = NA) +
  
  geom_line(linewidth = 1.1) +       
  geom_point(size = 2) +              # size OK for points
  facet_wrap(lact_group, scales = "free_y") +
  
  labs(
    x = "Month",
    y = "97th percentile DIM at BRED1",
    title = "DIM at BRED1 by Lactation Group",
    subtitle = unique(dim_at_bred$location_event)
  ) +
