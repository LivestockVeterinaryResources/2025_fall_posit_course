...
format: html
---
# Added install.packages("skimr")
#Added dplyr in library
...
counts <- count(animals,breed,sex,location_list)
...
animals_summary <- summary(animals)
summary(animals)
...
```{r}
#| label: extension
count(animals,date_sold[NA], sort=TRUE)
count(animals,location_list, sort=TRUE)
count(animals,location_list, date_died[NA], sort=TRUE)
```


```{r}
sum(is.na(animals["date_died"]))
skim_animals[[8,3]]
