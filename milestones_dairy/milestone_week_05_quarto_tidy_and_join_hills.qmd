...
quarto::quarto_render(
  ...
  execute_dir = dirname("../step3_denominators_by_lactation_group.qmd"),
  execute_params = list(
    denominator_granularity = 365,   # 365 day time period
    cut_by_days = 15,               # 15-day day-of-phase bins
    top_cut = 350,                  # DIM capped at 350 days
    top_cut_hfr = 450               # Heifer age capped at 450 days
  )
)
...
```{r}
denoNEW<-read_parquet('../data/intermediate_files/denominator_by_lact_group_time_period365.parquet')
```

Use this code chunk to explore the new denominator

```{r}
summary_table <- denoNEW %>%
  group_by(`Lactation Group`, date_time_period_start, date_time_period_end) %>%
  summarise(
    total_ct_animal_time_periods = sum(ct_animal_time_periods, na.rm = TRUE),
    total_ct_animals = sum(ct_animals, na.rm = TRUE),
    total_ct_animal_lactations = sum(ct_animal_lactations, na.rm = TRUE),
    .groups = "drop"
  )
