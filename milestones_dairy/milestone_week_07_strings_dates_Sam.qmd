---
title: "Week 7 Milestone Factors Waddell"
author: "<img src='images/DDH.png' width='50' /> <br> Dakota Dairy Health"
date: "`r Sys.Date()`"
format:
  html:
    theme: flatly
...
A few notes:  1) I put the code on how to insert an image in the header in the yaml change it to use your logo. 2)Any data (including farm name) that is reported in the text should use an inline text code chunk --> {12.18.2025}
```
...
'r farm_name' 
 ...
 # ^^ can change the data pulled in each report, which is nice when making a template - this only works if there's a "farm_name" function listed above
...
solution_data <- events_formatted_all %>% 
  mutate( 
    id_animal = str_c(source_file_path, ID, BDAT, sep= "_"), 
    location_event = str_replace(source_file_path, "Herd ", "data/event_files/"), #removes parts of the column contents in quotes 
    #location_event = (str_sub(start=1,end = 5) <-"Herd "), 
    remark_letters_1 = str_remove_all(Remark, "[^A-Za-z]+"), #The plus says go until there's a non-character, the ^ says to anchor at the beginning, and inside the bracket tells R what to match to, e.g. all letters either uppercase -A-Z- and 
    protocols_letters_1 = str_extract(Protocols, "^\\S+"), 
    FDAT = lubridate::mdy(FDAT), ##Forces lubridate, lubridate will generate "NA"s if it runs properly, but other date functions can override if not forced
