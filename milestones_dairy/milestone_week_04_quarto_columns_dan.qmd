...
events_health <- filter(events, event_type %in% 'health') #|> 
  events_health
  
  unique_health_events <- unique(events_health$event)
  unique_health_events
  ...
  # subset(event == 'NAVEL' | event == 'SEPTIC') |> 
...
# unique(events$protocols)
# unique(events$remark)
# unique(events$Remark)
# unique(events$remark_letters1)
...
events<-read_parquet('../data/intermediate_files/events_formatted.parquet') |> 
  ...
  mutate(
      Treatment = case_when(
          remark == "EXC+PEN" ~ "EXC+PEN",
          remark == "BAY+PEN" ~ "BAY+PEN",
          remark == "EXCNL+PEN" ~ "EXCNL+PEN",
          str_detect(remark, "PEN") ~ "PEN",
          str_detect(remark, "PLY") ~ "PLY",
          str_detect(remark, "X") ~ "X"),
          .default = NA) |> 
    
          ggplot(aes(floordate_month))+
          geom_bar(mapping = aes(color = Treatment))+
