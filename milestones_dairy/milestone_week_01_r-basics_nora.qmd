...
## Explore the Breed Variable



### list the values

I use this a whole lot when specifying levels of factors or mapping colors to 

```{r}

breeds<-sort(unique(animals$breed))
breeds
...
very fast easy code,  . . . not very pretty
...
Easier to look at when sorted and vertical
...
### data frame table in environment

This can be used later for standardization or graphing.
```{r}

table_breeds<-animals%>%count(breed, sort = TRUE)
