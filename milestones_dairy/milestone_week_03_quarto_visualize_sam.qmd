...
dim_at_bred <- 
events_formatted %>% 
  filter(floordate_month > as.Date("2022-10-01")) %>% 
  filter(event == "BRED1") %>% 
  mutate(dim_bred = (as.numeric(date_event-date_birth))) %>% 
  group_by(event,location_event,lact_group,floordate_month,dim_bred) %>% 
  summarize(
    mean = mean(dim_event),
    sd = sd(dim_event),
...
dim_at_bred %>% 
  ...
  facet_wrap(facets = vars(lact_group), scales = "free_y")
...
events_formatted %>% 
  ...
  filter(event == "HEAT1") %>% 
  mutate(dim_heat = as.numeric(date_event-date_birth))
dim_at_heat


# summary(
#    mean = mean(heat_to_bred),
#     sd = sd(heat_to_bred),
#     median = median(heat_to_bred),
#     min = min(heat_to_bred),
#     max = max(heat_to_bred),
#     q97 = quantile(heat_to_bred, probs = 0.97))
...
#   distinct(event)
#  mutate(
#     daysto_heat = as.numeric(date_heat-date_birth),  
#     daysto_conc = as.numeric(date_concieved), 
#     days_to_bred = (daysto_heat-daysto_conc)) %>% 
#    
#    mean = mean(days_to_bred),
#     sd = sd(days_to_bred),
#     median = median(days_to_bred),
#     min = min(days_to_bred),
#     max = max(days_to_bred),
#     q97 = quantile(days_to_bred, probs = 0.97))
...
events_formatted %>%
  ...
  filter(lact_group == "Heifer") %>%
  filter(event == "BRED1") %>%
  mutate(bred_date = as.Date(date_event),
    heat_to_bred = as.numeric(date_heat-bred_date)) 
dim_bred1
...
dim_bred1 %>%  
  ...
  #filter(event == "BRED1") %>% 
  #mutate(dim_bred = (as.numeric(date_event-date_birth))) %>% 
  group_by(location_event,lact_group,floordate_month,heat_to_bred) %>% 
  summary(
   mean = mean(heat_to_bred),
    sd = sd(heat_to_bred),
    median = median(heat_to_bred),
...
# new_bred <- dim_bred %>% 
#   left_join(dim_at_heat, by ="id_animal")
# new_bred

#joining two df -- just messing around 
#dim_bred$date_bred <- $value[match(df1$cow_id, df2$cow_id)]
```
