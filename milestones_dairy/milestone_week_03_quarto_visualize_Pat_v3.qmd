...
#Making a change for GH check

#malakdlafjdklflkasdjlkdfl fad
#alfjdlkfajlkfdkslfjl;kdsjfkl 
```
...
dim_at_bred <- events_formatted |>
  filter(event      == "BRED1",
         date_event >= max(date_event) - (365.25 * 3)) |>
...
dim_at_bred |> 
  ggplot(aes(floordate_month, q97, color = lact_group, fill = lact_group)) + 
    geom_point(size = 0.7) + 
    facet_wrap(vars(lact_group, location_event), scales = "free") +
  geom_smooth(span = 0.25, alpha = 0.3) 
