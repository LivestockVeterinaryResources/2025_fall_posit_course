...
check_data<-events_formatted%>%
  filter(event %in% 'BRED1')%>%
  filter(date_event>(max(date_event)-(365*3)))%>%
  select(id_animal, event, location_event, lact_group, floordate_month)%>%
  filter(floordate_month %in% lubridate::ymd('2025-10-01'))

dim_at_bred<-events_formatted%>%
  #filter(id_animal %in% get_example_cows)%>% #filter data to a small set of cows for testing
  filter(event %in% 'BRED1')%>%
  filter(date_event>(max(date_event)-(365*3)))%>%
...
ggplot(dim_at_bred)+
  ...
  facet_wrap(lact_group~., scales = 'free')
...
find_peaks<-dim_at_bred%>%
  filter(q97>(mean+(sd*3.8)))

df_peaks<-events_formatted%>%
  filter(event %in% 'BRED1')%>%
  filter(date_event>(max(date_event)-(365*3)))%>%
  filter(floordate_month %in% find_peaks$floordate_month)%>%
  left_join(dim_at_bred)%>%
  mutate(peak_point = case_when(
    dim_event>q97~'Peak', 
    TRUE~'OK'
  ))
...
ggplot(df_peaks)+
  geom_point(aes(x = floordate_month, y = dim_event, color = peak_point), 
             position = position_jitter())+
  geom_hline(data = dim_at_bred%>%
  filter(floordate_month %in% find_peaks$floordate_month), 
             aes(yintercept = q97))+
  facet_wrap(lact_group~., scales = 'free')+
...
```{r}
dim_at_mast<-events_formatted%>%
  #filter(id_animal %in% get_example_cows)%>% #filter data to a small set of cows for testing
  filter(event %in% c('MAST1', 'MAST2', 'MAST3', 'MAST4'))%>%
  filter(date_event>(max(date_event)-(365*3)))%>%
  group_by(event, location_event, lact_group, floordate_month)%>%
  summarize(mean = mean(dim_event),
            sd = sd(dim_event),
