...
dim_at_bred <- events_formatted %>%
  ...
  filter(date_event > max(date_event) - 
          # I used this to account for leap years.
         lubridate::years(3)
         # to get same answers as solution use
         # 365*3
         ) %>%
...
dim_fb <- dim_at_bred |> 
  ...
  labs(title = "2023 had a higer DIM at FB",
       x = "Month Bred",
       y = "97th Percentile of DIM at First Bred",
       color = "") +
  theme_minimal() +
  theme(
...
dim_lame <- events_formatted %>%
  ...
  select(location_event, lact_group, protocols, dim_event, date_event) 
...
```{r}
#| label: extension-plot

plot <- dim_lame |> 
  mutate(protocols = as_factor(protocols),
         protocols = fct_infreq(protocols)
          )|>
  ggplot(aes(x = dim_event, fill = protocols)) +
  geom_histogram(position = "dodge", bins = 30) +
  facet_grid(rows = vars(protocols), cols = vars(location_event,
                                                 lact_group)
             )+
  scale_fill_viridis_d() +
  labs(title = "Hairy Wart is most common protocol for Lameness",
       x = "Days in Milk at First Lame Event",
       y = "Count of First Lame Events"
       ) +
  theme_minimal() +
  theme(
    legend.position = "none"
