...
```{r}
## There is a unique identifier and it's the animal_id and animal_id_lactation (for just the lact)
deno365 %>% 
count(deno_type, sort = TRUE)
```
...
deno365_long <-
  deno365 %>% 
  pivot_longer( 
  cols = c(ct_animal_time_periods, ct_animals , ct_animal_lactations), 
  names_to = "metric", 
  values_to = "ct")
print(deno365_long)
...
quarto::quarto_render(
  ...
  execute_params = list(
    denominator_granularity = 30, #modify this number to create time periods of different lengths (i.e. try  21 or 90 days)
      ...
      top_cut = 180, #What is the max DIM you care about? this puts a ceiling on the number of categories so when you have an outlier it doesn't give you too many groups
...
```{r}
denoNEW<-read_parquet('../data/intermediate_files/denominator_by_lact_group_time_period90.parquet')
```
...
denoNEW %>% 
  filter(deno_type == "lact_basic") %>% #only allows lact_basic denom
  ggplot(aes(x = date_time_period_start, 
             y = ct_animal_time_periods,
             color = date_time_period_end,
             size=ct_animals))+
  geom_point() +
  facet_wrap(~ `Lactation Group`) +
  labs(title = "Denominators by Lact")
```
...
# animals_full <-animals %>% 
#anti_join(animal_lactations) #tells you if there's differences 
  
animals_full <- 
  animals %>% 
left_join(animal_lactations %>% 
            select(id_animal)) %>% 
            distinct()
